...
public class FusionSnake : MonoBehaviour, ISnake {
    ...
    for (int i = 0; i < hits.Length; i++){
		...
		for (int i = 0; i < hits.Length; i++){
			if(hits[i].collider.name.StartsWith("Food")){
				foodDistNorthEast = hits[i].distance;
			}
			if(hits[i].collider.name.StartsWith("Tail")){
		...
		for (int i = 0; i < hits.Length; i++){
			if(hits[i].collider.name.StartsWith("Food")){
				foodDistSouthEast = hits[i].distance;
			}
			if(hits[i].collider.name.StartsWith("Tail")){
		...
		for (int i = 0; i < hits.Length; i++){
			if(hits[i].collider.name.StartsWith("Food")){
				foodDistSouthWest = hits[i].distance;
			}
			if(hits[i].collider.name.StartsWith("Tail")){
		...
		Debug.DrawRay(transform.position, northWest*10, Color.green);

		for (int i = 0; i < hits.Length; i++){
			if(hits[i].collider.name.StartsWith("Food")){
				foodDistNorthWest = hits[i].distance;
			}
			if(hits[i].collider.name.StartsWith("Tail")){
		...
		float[] z = {foodDistNorth/scaleFactor,foodDistNorthEast/scaleFactor,foodDistEast/scaleFactor,foodDistSouthEast/scaleFactor,foodDistSouth/scaleFactor,foodDistSouthWest/scaleFactor,foodDistWest/scaleFactor,foodDistNorthWest/scaleFactor,wallDistNorth/scaleFactor,wallDistNorthEast/scaleFactor,wallDistEast/scaleFactor,wallDistSouthEast/scaleFactor,wallDistSouth/scaleFactor,wallDistSouthWest/scaleFactor,wallDistWest/scaleFactor,wallDistNorthWest/scaleFactor,bodyDistNorth/scaleFactor,bodyDistNorthEast/scaleFactor,bodyDistEast/scaleFactor,bodyDistSouthEast/scaleFactor,bodyDistSouth/scaleFactor,bodyDistSouthWest/scaleFactor,bodyDistWest/scaleFactor,bodyDistNorthWest/scaleFactor};
