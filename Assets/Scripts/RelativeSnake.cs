...
public class RelativeSnake : MonoBehaviour, ISnake {
	...
	void Inputs(){
		...
		Ray wallf = new Ray(transform.position,transform.forward);
		Ray wallr = new Ray(transform.position,transform.right);
		Ray walll = new Ray(transform.position,-transform.right);
		Ray wallb = new Ray(transform.position,-transform.forward);
		RaycastHit hit;
